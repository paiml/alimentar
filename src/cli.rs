...
use crate::{
    ...
    split::DatasetSplit,
    tui::{DatasetAdapter, DatasetViewer},
...
pub fn run() -> ExitCode {
    ...
    let result = match cli.command {
        ...
        Commands::View { path, search } => cmd_view(&path, search.as_deref()),
        #[cfg(feature = "hf-hub")]
...
fn cmd_view(path: &PathBuf, initial_search: Option<&str>) -> crate::Result<()> {
...
fn prompt_search<W: Write>(stdout: &mut W) -> crate::Result<Option<String>> {
    use crossterm::{
        cursor,
